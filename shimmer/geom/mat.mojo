...
@fieldwise_init
struct Mat4(Copyable, ImplicitlyCopyable, Movable):
    var x_axis: Vec4
    var y_axis: Vec4
    var z_axis: Vec4
    var w_axis: Vec4

    comptime ZERO = Self(Vec4.ZERO, Vec4.ZERO, Vec4.ZERO, Vec4.ZERO)
    """
    A 4x4 matrix with all elements set to `0.0`.
    """

    comptime IDENTITY = Self(Vec4.X, Vec4.Y, Vec4.Z, Vec4.W)
    """
    ...
    fn to_cols_array(self) -> InlineArray[Float32, 16]:
        """
        Creates a `StaticTuple[Float32, 16]` array storing data in column major order.
        If you require data in row major order `transpose` the matrix first.
        """
        return InlineArray[Float32, 16](
            self.x_axis.x,
            self.x_axis.y,
            self.x_axis.z,
            self.x_axis.w,
    ...
    fn to_cols_array_2d(self) -> InlineArray[InlineArray[Float32, 4], 4]:
        """
        Creates a 2D array storing data in column major order.
        If you require data in row major order `transpose` the matrix first.
        """
        return InlineArray[InlineArray[Float32, 4], 4](
    ...
    fn perspective_lh(
        ...
        var sin_fov = sin(0.5 * fov_y_radians)
        var cos_fov = cos(0.5 * fov_y_radians)
        var h = cos_fov / sin_fov
        var w = h / aspect_ratio
        var r = z_far / (z_far - z_near)
        return Self(
            Vec4(w, 0.0, 0.0, 0.0),
            Vec4(0.0, h, 0.0, 0.0),
            Vec4(0.0, 0.0, r, 1.0),
    ...
    fn perspective_rh_gl(
        ...
        var inv_length = 1.0 / (z_near - z_far)
        var f = 1.0 / tan(0.5 * fov_y_radians)
        var a = f / aspect_ratio
        var b = (z_near + z_far) * inv_length
        var c = (2.0 * z_near * z_far) * inv_length
        return Self(
            Vec4(a, 0.0, 0.0, 0.0),
            Vec4(0.0, f, 0.0, 0.0),
            Vec4(0.0, 0.0, b, -1.0),
    ...
    fn perspective_infinite_lh(
        fov_y_radians: Float32, aspect_ratio: Float32, z_near: Float32
    ) -> Self:
        """
    ...
    fn orthographic_lh(
        ...
        var r = 1.0 / (far - near)
        return Self(
            Vec4(rcp_width + rcp_width, 0.0, 0.0, 0.0),
            Vec4(0.0, rcp_height + rcp_height, 0.0, 0.0),
            Vec4(0.0, 0.0, r, 0.0),
            Vec4(
    ...
    fn orthographic_rh_gl(
        ...
        This is the same as the OpenGL `glOrtho` function.
    ...
    fn frustum_lh(
        ...
        Creates a left-handed perspective projection matrix with `[0,1]` depth range.
        """
        var inv_width = 1.0 / (right - left)
        var inv_height = 1.0 / (top - bottom)
        var inv_depth = 1.0 / (z_far - z_near)
        var a = (right + left) * inv_width
        var b = (top + bottom) * inv_height
        var c = z_far * inv_depth
        var d = -(z_far * z_near) * inv_depth
        var two_z_near = 2.0 * z_near
        return Self(
            Vec4(two_z_near * inv_width, 0.0, 0.0, 0.0),
            Vec4(0.0, two_z_near * inv_height, 0.0, 0.0),
            Vec4(a, b, c, 1.0),
            Vec4(0.0, 0.0, d, 0.0),
        )
    ...
    fn frustum_rh_gl(
        ...
        var a = (right + left) * inv_width
        var b = (top + bottom) * inv_height
        var c = -(z_far + z_near) * inv_depth
        var d = -(2.0 * z_far * z_near) * inv_depth
        var two_z_near = 2.0 * z_near
        return Self(
            Vec4(two_z_near * inv_width, 0.0, 0.0, 0.0),
            Vec4(0.0, two_z_near * inv_height, 0.0, 0.0),
    ...
    fn transform_point3(self, rhs: Vec3) -> Vec3:
        ...
        """
        var res = self.x_axis * rhs.x
        res = self.y_axis * rhs.y + res
        res = self.z_axis * rhs.z + res
        res = self.w_axis + res
    ...
    @staticmethod
    fn from_rotation_x(angle: Float32) -> Self:
        """
    ...
    @staticmethod
    fn from_rotation_y(angle: Float32) -> Self:
        """
    ...
    @staticmethod
    fn from_rotation_z(angle: Float32) -> Self:
        """
    ...
    @staticmethod
    fn from_axis_angle(axis: Vec3, angle: Float32) -> Self:
        """
        ...
        var sina = sin(angle)
        var cosa = cos(angle)
        var axis_sin = axis * sina
        var axis_sq = axis * axis
        var omc = 1.0 - cosa
        var xyomc = axis.x * axis.y * omc
        var xzomc = axis.x * axis.z * omc
        var yzomc = axis.y * axis.z * omc
        return Self(
            Vec4(
                axis_sq.x * omc + cosa,
                xyomc + axis_sin.z,
    ...
    fn col(self, index: Int) -> Vec4:
        ...
        if index == 0:
            ...
            # TODO: Use proper panic/assert when available
            return Vec4.ZERO

    fn row(self, index: Int) -> Vec4:
        """
    ...
    fn __mul__(self, rhs: Vec4) -> Vec4:
        """Transforms a 4D vector."""
        var res = self.x_axis * rhs.x
        res = res + (self.y_axis * rhs.y)
        res = res + (self.z_axis * rhs.z)
        res = res + (self.w_axis * rhs.w)
        return res
    ...
    fn determinant(self) -> Float32:
        ...
        var a00 = self.x_axis.x
        var a01 = self.x_axis.y
        var a02 = self.x_axis.z
        var a03 = self.x_axis.w
        var a10 = self.y_axis.x
        var a11 = self.y_axis.y
        var a12 = self.y_axis.z
        var a13 = self.y_axis.w
        var a20 = self.z_axis.x
        var a21 = self.z_axis.y
        var a22 = self.z_axis.z
        var a23 = self.z_axis.w
        var a30 = self.w_axis.x
        var a31 = self.w_axis.y
        var a32 = self.w_axis.z
        var a33 = self.w_axis.w
    ...
    # comptime NAN: Self = Self::from_cols(Vec4::NAN, Vec4::NAN, Vec4::NAN, Vec4::NAN);
    # """
    # All NAN:s.
    # """
